...
```powershell
pwsh -NoProfile -File ./tools/Publish-LauncherRelease.ps1 -Version 1.1.0
```
...
push them with a `launcher-v1.1.0` tag, wait for the workflow to publish the
...
```powershell
git add -f release-assets/Project-Rebirth-Launcher-1.1.0-win-x64.zip
git add -f release-assets/Project-Rebirth-Launcher-1.1.0-win-x64.zip.sha256
git commit -m "Stage launcher 1.1.0 release assets"
git tag launcher-v1.1.0
git push origin main launcher-v1.1.0
```
